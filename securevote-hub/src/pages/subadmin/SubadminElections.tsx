...
export default function SubadminElections() {
  const { user } = useAuth()
  ...
  const onCreate = async (data: ElectionForm) => {
    try {
      ...
      // Send the Sub-Admin's organizationName automatically
  ...
  const toggleElection = async (id: string, action: "start" | "end") => {
    try {
      ...
      toast({
        title: action === "start" ? "Election started" : "Election ended"
      })
      await loadElections()
    } catch (err: any) {
      toast({
    ...
    <DashboardLayout role="subadmin">
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Manage blockchain elections</p>
        </div>
      ...
      <div className="space-y-4">
        {pageLoading && <p className="text-center text-muted-foreground">Loading elections...</p>}
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <div className="mt-1 flex items-center gap-3 text-xs text-muted-foreground">
                      ...
                      <span className={renderStatusColor(el.status)}>{el.status}</span>
                    </div>
                  </div>
                </div>

                {/* Sub-admins are authorized to start/end elections */}
                    ...
                    <>
                      {el.status === "upcoming" && (
                        <Button
                          ...
                          onClick={() => toggleElection(el.id, "start")}
                        >
                      ...
                      {el.status === "active" && (
                        <Button
                          ...
                          variant="destructive"
                          disabled={loadingId === el.id}
                          onClick={() => toggleElection(el.id, "end")}
                        >
